...
在实际应用中，我们不可能精确获得数据的真实得分函数 \( s(x, t) = \nabla_x \log p_t(x) \)，因此需要用神经网络 \( s_\theta(x, t) \) 进行近似。然而，高维情况下，得分函数的估计误差可能会放大，导致生成轨迹不稳定。噪声影响较大，如果训练数据不足，得分函数可能会学到错误的梯度方向。为了缓解得分函数的误差，可以采用以下方法：

1. 正则化得分函数：通过对抗训练（Adversarial Training）或平滑方法（如 Stein Score Matching）提高得分函数的稳定性。
  
1. 改进神经网络架构：使用更强的 Transformer 或 Diffusion U-Net 结构，提高得分函数的拟合能力。

1. 调整训练数据分布：通过重要性采样（Importance Sampling）或流形学习（Manifold Learning）优化得分函数估计。

## 5 解析计算与物理机制

### 5.1 解析计算

为了分析扩散模型的反向生成过程，我们考虑一个简单但具有代表性的概率分布：高斯混合模型（GMM）。设数据分布由两个高斯簇组成，每个簇的均值分别为 \( \pm m \)：
\[
P_0(x) = \frac{1}{2} \mathcal{N}(m, I) + \frac{1}{2} \mathcal{N}(-m, I)
\]
其中：设数据维度为 \( d \)，则均值 \( m \) 的范数满足 \( |m|^2 = d\mu^2 \)，表示高斯簇在高维空间中的分布中心。每个簇的方差设为 \( \sigma^2 I \)，其中 \( I \) 为单位矩阵。根据扩散过程的理论，物种分化时间 \( t_S \) 由数据协方差矩阵的最大特征值 \( \Lambda \) 决定：
\[
\Lambda e^{-2t_S} = 1
\]
在 GMM 中，数据协方差矩阵的主成分（即最大的特征值）可以近似为：
\[
\Lambda \approx d\mu^2
\]
因此，分化时间 \( t_S \) 近似为：
\[
t_S = \frac{1}{2} \log d
\]
这表明，维度 \( d \) 越高，物种分化发生得越晚，这与维度诅咒一致。

记忆塌缩时间 \( t_C \) 由数据分布的熵决定。设数据点的分布熵为 \( s(t) \)，分离数据点的熵为 \( s_{\text{sep}}(t) \)。记忆塌缩发生在 \( s(t_C) = s_{\text{sep}}(t_C) \)。

在 GMM 中，可以推导出：
\[
t_C = \frac{1}{2} \log \left( 1 + \frac{\sigma^2}{n^{2/d} - 1} \right)
\]
其中：\( n \) 为训练数据点数，\( d \) 为数据维度。表明，当 \( n \ll e^d \) 时，\( t_C \) 很小，即模型容易塌缩到训练数据。

### 5.2 物理机制解释

物种分化 = 对称性破缺

在高斯混合模型中，初始数据点是从高斯分布中均匀采样的，没有类别信息。反向扩散初期，数据点在高维空间中随机漫步（布朗运动）。当 \( t = t_S \) 时，数据点开始沿着主成分方向聚类，即数据点不再是均匀分布，而是偏向某些类别。数学上，这对应于协方差矩阵的主特征值分裂，类似于物理中的自发对称性破缺（SSB, Spontaneous Symmetry Breaking）。对称相（Symmetric Phase）：在 \( t > t_S \) 时，数据点仍然是无结构的高斯分布（高熵）。破缺相（Broken Phase）：在 \( t < t_S \) 时，数据点开始分化，并形成类别结构（低熵）。

记忆塌缩 = 玻璃相转变

在 \( t < t_C \) 之前，生成数据点可以覆盖整个数据空间（液体相）。当 \( t = t_C \) 时，数据点塌缩到训练数据点，形成离散的吸引子（玻璃态）。训练数据点相当于玻璃系统中的局部能量极小点。记忆塌缩可以类比于自旋玻璃相变，即高温下的流体状态（随机分布）冻结成低温下的分离团簇（塌缩到训练数据）。

这为理解扩散模型的动态机制提供了强有力的物理基础。
